...
for batch_file_paths in nav_file_paths:
    ...
    for path in batch_file_paths:
        ...
        nav_data = nav_data.sort_values(by="日期", ascending=True).reset_index(
            drop=True
        )
        # 选取最大的开始时间作为开始时间
    ...
    if html_name == "" and len(batch_file_paths) > 1:
        html_name = (
            "compare_"
            + datetime.date.today().strftime("%Y%m%d")
            + "_"
            + batch_file_paths[0].stem
            + "_etc_nav_analysis"
        )
    elif html_name == "":
        html_name = (
        ...
        pass
    html_file_path = batch_file_paths[0].parent.joinpath(f"{html_name}.html")
